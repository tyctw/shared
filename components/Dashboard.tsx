...
const Dashboard: React.FC<DashboardProps> = ({ entries }) => {
  ...
  const stats = useMemo(() => {
      ...
      const pointsValues = filteredEntries
          .map(entry => Number(entry.totalPoints))
          .filter(value => Number.isFinite(value))
          .sort((a, b) => a - b);
      const averagePoints = pointsValues.length
          ? Number((pointsValues.reduce((sum, value) => sum + value, 0) / pointsValues.length).toFixed(2))
          : 0;
      const medianPoints = pointsValues.length ? pointsValues[Math.floor(pointsValues.length / 2)] : 0;
      const lowestPoints = pointsValues.length ? pointsValues[0] : 0;
      const highestPoints = pointsValues.length ? pointsValues[pointsValues.length - 1] : 0;
      ...
      const scoreBands = [
          ...
          { name: '51+', min: 51, max: Number.POSITIVE_INFINITY },
      ].map(band => ({
          name: band.name,
          count: pointsValues.filter(value => value >= band.min && value <= band.max).length,
      }));

      const departmentCounts: Record<string, number> = {};
      const identityCounts: Record<string, number> = {};
      const writingCounts: Record<string, number> = {};
      for (let grade = 0; grade <= 6; grade += 1) writingCounts[String(grade)] = 0;

      const aCountDistribution: Record<string, number> = { '5A': 0, '4A': 0, '3A': 0, '2A': 0, '1A': 0, '0A': 0 };
      const subjectHighStats: Record<string, { label: string; high: number; aOrBetter: number; total: number }> = {
          chinese: { label: '國文', high: 0, aOrBetter: 0, total: 0 },
          english: { label: '英文', high: 0, aOrBetter: 0, total: 0 },
          math: { label: '數學', high: 0, aOrBetter: 0, total: 0 },
          nature: { label: '自然', high: 0, aOrBetter: 0, total: 0 },
          social: { label: '社會', high: 0, aOrBetter: 0, total: 0 },
      };

      filteredEntries.forEach(entry => {
          const department = entry.department || '未填科系';
          departmentCounts[department] = (departmentCounts[department] || 0) + 1;
          identityCounts[entry.studentIdentity] = (identityCounts[entry.studentIdentity] || 0) + 1;
          writingCounts[String(entry.scores.writing)] = (writingCounts[String(entry.scores.writing)] || 0) + 1;

          const subjects = [entry.scores.chinese, entry.scores.english, entry.scores.math, entry.scores.nature, entry.scores.social];
          const aCount = subjects.filter(score => String(score).startsWith('A')).length;
          aCountDistribution[`${aCount}A`] = (aCountDistribution[`${aCount}A`] || 0) + 1;

          ([
              ['chinese', entry.scores.chinese],
              ['english', entry.scores.english],
              ['math', entry.scores.math],
              ['nature', entry.scores.nature],
              ['social', entry.scores.social],
          ] as const).forEach(([key, score]) => {
              subjectHighStats[key].total += 1;
              if (score === 'A++' || score === 'A+') subjectHighStats[key].high += 1;
              if (String(score).startsWith('A')) subjectHighStats[key].aOrBetter += 1;
          });
      });
      ...
      const writingData = Object.entries(writingCounts).map(([name, count]) => ({ name: `${name}級`, count }));
      const aCountData = ['5A', '4A', '3A', '2A', '1A', '0A'].map(name => ({ name, count: aCountDistribution[name] || 0 }));
      const subjectHighData = Object.values(subjectHighStats).map(subject => ({
          subject: subject.label,
          highRate: subject.total ? Number(((subject.high / subject.total) * 100).toFixed(1)) : 0,
          aRate: subject.total ? Number(((subject.aOrBetter / subject.total) * 100).toFixed(1)) : 0,
      }));
      const regionBenchmarkData = Object.entries(
          filteredEntries.reduce<Record<string, { sum: number; count: number }>>((map, entry) => {
              if (!map[entry.region]) map[entry.region] = { sum: 0, count: 0 };
              map[entry.region].sum += entry.totalPoints;
              map[entry.region].count += 1;
              return map;
          }, {})
      )
          .map(([name, value]) => ({ name, avgPoints: Number((value.sum / value.count).toFixed(2)), count: value.count }))
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {[
                ['平均總積分', stats.averagePoints],
                ['中位總積分', stats.medianPoints],
                ['最低紀錄', stats.lowestPoints],
                ['最高紀錄', stats.highestPoints],
            ].map(([label, value]) => (
                <div key={label} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                    <p className="text-[11px] font-black text-slate-400">{label}</p>
                    <p className="mt-1 font-mono text-2xl font-black text-slate-800">{value}</p>
                </div>
            ))}
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm">
                ...
                <p className="mb-6 text-sm font-medium text-slate-500">快速看目前資料集中落在哪些總積分帶，適合判斷樣本集中度。</p>
                <div className="h-72 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={stats.scoreBands} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            <defs>
                                <linearGradient id="scoreBandGradient" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <Tooltip content={<CustomBarTooltip />} cursor={{ stroke: '#06b6d4', strokeWidth: 1 }} />
                            <Area type="monotone" dataKey="count" name="筆數" stroke="#06b6d4" strokeWidth={3} fill="url(#scoreBandGradient)" />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm">
                ...
                <p className="mb-6 text-sm font-medium text-slate-500">把國英數自社轉成 5A 到 0A，方便和落點討論中常見的門檻語言對齊。</p>
                <div className="h-72 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={stats.aCountData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 13, fontWeight: 700}} dy={10} />
                            <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 12}} />
                            <Tooltip content={<CustomBarTooltip />} cursor={{fill: '#fef3c7', radius: 8}} />
                            <Bar dataKey="count" radius={[8, 8, 8, 8]} barSize={42}>
                                {stats.aCountData.map((entry, index) => (
                                    <Cell key={`a-count-${entry.name}`} fill={['#f59e0b', '#f97316', '#ef4444', '#8b5cf6', '#3b82f6', '#94a3b8'][index]} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="xl:col-span-2 bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm">
                <h4 className="font-bold text-slate-700 mb-2 flex items-center gap-2">
                    <span className="w-1.5 h-6 bg-emerald-500 rounded-full"></span>
                    各科高分率比較
                </h4>
                <p className="mb-6 text-sm font-medium text-slate-500">A 以上代表穩定高表現；A+ 以上可用來觀察尖端分布。</p>
                <div className="h-80 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={stats.subjectHighData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                            <XAxis dataKey="subject" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 13, fontWeight: 700}} dy={10} />
                            <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 12}} tickFormatter={(value) => `${value}%`} />
                            <Tooltip content={<CustomLineTooltip />} cursor={{fill: '#f1f5f9', radius: 8}} />
                            <Legend iconType="circle" wrapperStyle={{ fontSize: '12px', fontWeight: 700 }} />
                            <Bar dataKey="aRate" name="A以上%" fill="#10b981" radius={[8, 8, 8, 8]} />
                            <Bar dataKey="highRate" name="A+以上%" fill="#6366f1" radius={[8, 8, 8, 8]} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm">
                ...
                <p className="mb-6 text-sm font-medium text-slate-500">觀察作文在樣本中的集中級分。</p>
                <div className="h-80 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={stats.writingData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 700}} dy={10} />
                            <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 12}} />
                            <Tooltip content={<CustomBarTooltip />} cursor={{fill: '#fff1f2', radius: 8}} />
                            <Bar dataKey="count" fill="#f43f5e" radius={[8, 8, 8, 8]} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm">
                ...
                <div className="space-y-4">
                    {stats.departmentData.map((department, index) => (
                        <div key={department.name}>
                            <div className="mb-1 flex items-center justify-between gap-3 text-sm font-bold">
                                <span className="min-w-0 truncate text-slate-700" title={department.name}>{index + 1}. {department.name}</span>
                                <span className="shrink-0 text-slate-500">{department.count} 筆</span>
                            </div>
                            <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                                <div className="h-full rounded-full bg-violet-500" style={{ width: `${stats.departmentData[0] ? (department.count / stats.departmentData[0].count) * 100 : 0}%` }} />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm">
                ...
                <div className="h-72 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={stats.identityData} innerRadius={58} outerRadius={82} paddingAngle={4} dataKey="value" stroke="none">
                                {stats.identityData.map((entry, index) => (
                                    <Cell key={`identity-${entry.name}`} fill={COLORS_PIE[index % COLORS_PIE.length]} />
                                ))}
                            </Pie>
                            <Tooltip content={<CustomPieTooltip />} />
                            <Legend verticalAlign="bottom" height={42} iconType="circle" wrapperStyle={{ fontSize: '12px', fontWeight: 700 }} />
                        </PieChart>
                    </ResponsiveContainer>
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm">
                ...
                <div className="space-y-4">
                    {stats.regionBenchmarkData.map((region, index) => (
                        <div key={region.name}>
                            <div className="mb-1 flex items-center justify-between gap-3 text-sm font-bold">
                                <span className="min-w-0 truncate text-slate-700" title={region.name}>{index + 1}. {region.name}</span>
                                <span className="shrink-0 text-slate-500">{region.avgPoints} / {region.count}筆</span>
                            </div>
                            <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                                <div className="h-full rounded-full bg-sky-500" style={{ width: `${stats.regionBenchmarkData[0] ? (region.avgPoints / stats.regionBenchmarkData[0].avgPoints) * 100 : 0}%` }} />
                            </div>
                        </div>
                    ))}
                </div>
